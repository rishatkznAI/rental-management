...
test('service photos from protected uploads render without cross-origin console errors', async ({ page }) => {
  ...
  try {
    ...
    const seed = await withAdminApi(async (api) => {
      ...
      const response = await api.post('/api/service', {
        data: {
          ...
          photos: [{ localPath: uploadPublicPath, originalUrl: 'https://cdn.example.test/original-before.webp' }],
          repairPhotos: {
            before: [{ localPath: uploadPublicPath, originalUrl: 'https://cdn.example.test/original-before.webp' }],
            after: [],
    ...
    fs.rmSync(uploadTicketDir, { recursive: true, force: true });
  }
});
