...
A principal with no active membership, more than one active membership, or an inactive/missing company has no complete actor scope. Authentication may still succeed for existing read-only compatibility, but every scoped write fails before mutation with `ACTOR_SCOPE_INCOMPLETE`.
...
This is expressed centrally as the `company_is_tenant` model in `trusted-actor-scope.js`; it is not a default identifier. A future multi-company Tenant aggregate must change this resolver and its membership model rather than add route-level fallbacks.
...
Persistent sessions therefore keep scope across reloads, new requests, and backend restart. Membership removal, ambiguity, or ownership change removes usable scope for subsequent writes.
...
Actor scope does not replace authorization. Backend authentication, collection/role permission checks, and actor-scope-to-entity-scope comparison remain separate requirements. An administrator from one company cannot mutate master data owned by another company or tenant.
