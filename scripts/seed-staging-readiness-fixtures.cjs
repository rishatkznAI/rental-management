...
const { DB_PATH, getData, setData } = serverRequire('./db');
...
function assertStagingFixtureSeedAllowed(env = process.env) {
  ...
  if (!isStagingLike(env)) {
    throw new Error('Refused: environment is not clearly staging.');
  }
  if (isProductionLike(env)) {
    throw new Error('Refused: environment looks production-like.');
  }
...
function buildFixtures(now = new Date()) {
  ...
  const equipment = [
    {
      id: `${PREFIX}EQ-READY`,
      inventoryNumber: `${PREFIX}INV-READY`,
      serialNumber: `${PREFIX}SN-READY`,
      manufacturer: 'Skytech Test',
      model: 'Ready Lift 10',
      status: 'available',
      notes: 'STAGING TEST FIXTURE: fleet readiness ready state.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      id: `${PREFIX}EQ-RENTED`,
      inventoryNumber: `${PREFIX}INV-RENTED`,
      serialNumber: `${PREFIX}SN-RENTED`,
      manufacturer: 'Skytech Test',
      model: 'Rental Lift 12',
      status: 'rented',
      notes: 'STAGING TEST FIXTURE: active rental state.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      id: `${PREFIX}EQ-SERVICE`,
      inventoryNumber: `${PREFIX}INV-SERVICE`,
      serialNumber: `${PREFIX}SN-SERVICE`,
      manufacturer: 'Skytech Test',
      model: 'Service Lift 14',
      status: 'in_service',
      plannedMonthlyRevenue: 210000,
      notes: 'STAGING TEST FIXTURE: open service state.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      id: `${PREFIX}EQ-DELIVERY`,
      inventoryNumber: `${PREFIX}INV-DELIVERY`,
      serialNumber: `${PREFIX}SN-DELIVERY`,
      manufacturer: 'Skytech Test',
      model: 'Delivery Lift 16',
      status: 'available',
      plannedMonthlyRevenue: 240000,
      notes: 'STAGING TEST FIXTURE: active delivery blocker.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      id: `${PREFIX}EQ-GSM`,
      inventoryNumber: `${PREFIX}INV-GSM`,
      serialNumber: `${PREFIX}SN-GSM`,
      manufacturer: 'Skytech Test',
      model: 'GSM Lift 18',
      status: 'available',
      gsmImei: `${PREFIX}IMEI-0001`,
      gsmLastSeenAt: staleIso,
      notes: 'STAGING TEST FIXTURE: stale GSM signal.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      id: `${PREFIX}EQ-CHECK`,
      inventoryNumber: `${PREFIX}INV-CHECK`,
      serialNumber: `${PREFIX}SN-CHECK`,
      manufacturer: 'Skytech Test',
      model: 'Return Check Lift 20',
      status: 'available',
      notes: 'STAGING TEST FIXTURE: missing return evidence.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      ...
      serialNumber: `${PREFIX}SN-DOC`,
      manufacturer: 'Skytech Test',
      model: 'Document Lift 22',
      status: 'available',
      plannedMonthlyRevenue: 150000,
      notes: 'STAGING TEST FIXTURE: document blocker.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
    {
      id: `${PREFIX}EQ-UNKNOWN`,
      inventoryNumber: `${PREFIX}INV-UNKNOWN`,
      serialNumber: `${PREFIX}SN-UNKNOWN`,
      manufacturer: 'Skytech Test',
      model: 'Legacy Lift 24',
      status: 'legacy_hold',
      notes: 'STAGING TEST FIXTURE: ambiguous legacy status.',
      fixtureTag: PREFIX,
      updatedAt: iso,
    },
  ];
...
if (require.main === module) {
  try {
    const result = seedStagingReadinessFixtures();
