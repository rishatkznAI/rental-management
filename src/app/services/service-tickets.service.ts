...
export function normalizeServiceTicketDto(value: unknown, index = 0): ServiceTicket | null {
  ...
  return {
    ...
    counterpartyId: stringValue(item.counterpartyId ?? item.counterparty_id) || undefined,
    counterpartyName: stringValue(item.counterpartyName) || undefined,
    customerDisplayName: stringValue(item.customerDisplayName) || undefined,
    clientId: stringValue(item.clientId ?? item.client_id) || undefined,
    client: stringValue(item.client ?? item.clientName) || undefined,
    clientName: stringValue(item.clientName ?? item.client) || undefined,
    rentalId: stringValue(item.rentalId ?? item.rental_id) || undefined,
    objectId: stringValue(item.objectId ?? item.clientObjectId ?? item.siteId) || undefined,
    contractId: stringValue(item.contractId ?? item.clientContractId) || undefined,
    objectName: stringValue(item.objectName) || undefined,
    objectAddress: stringValue(item.objectAddress) || undefined,
    objectContactName: stringValue(item.objectContactName) || undefined,
    objectContactPhone: stringValue(item.objectContactPhone) || undefined,
    contractNumber: stringValue(item.contractNumber) || undefined,
    source: item.source as ServiceTicket['source'],
    status: normalizeStatus(item.status),
    plannedDate: stringValue(item.plannedDate) || undefined,
    scheduledDate: stringValue(item.scheduledDate) || undefined,
    dueDate: stringValue(item.dueDate) || undefined,
    deadline: stringValue(item.deadline) || undefined,
    targetDate: stringValue(item.targetDate) || undefined,
    completedAt: stringValue(item.completedAt) || undefined,
    mechanicId: stringValue(item.mechanicId) || undefined,
    assignedUserId: stringValue(item.assignedUserId) || undefined,
    responsibleUserId: stringValue(item.responsibleUserId) || undefined,
