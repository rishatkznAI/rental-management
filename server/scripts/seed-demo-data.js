...
function buildDemoData(now = new Date('2026-05-03T09:00:00.000Z')) {
  ...
  const rentals = [
    ...
    },
    {
      ...
      contact: 'Илья Логистов',
      equipmentId: 'demo-eq-scissor-01',
      equipment: ['demo-eq-scissor-01'],
      equipmentModel: 'Genie GS-3246',
      equipmentName: 'Genie GS-3246',
      equipmentInventoryNumber: 'DEMO-INV-002',
      equipmentInv: 'DEMO-INV-002',
      manager: 'Demo Office',
      ...
      rate: '16 000 ₽ / день',
      discount: 0,
      deliveryAddress: 'Казань, демо-склад клиента',
      totalAmount: 96000,
      paidAmount: 0,
      history: [],
      createdAt: nowIso,
  ...
  const ganttRentals = rentals.map(item => ({
    ...
    managerInitials: item.manager.split(/\s+/).map(part => part[0]).join('').slice(0, 2).toUpperCase(),
