...
test('rental manager can create a service ticket from the service form', async ({ page }) => {
  const suffix = `svc-manager-${Date.now()}`;
  const { equipment } = await withAdminApi(async (api) => ({
    equipment: await getAnyRentableEquipment(api),
  }));
  const reason = `E2E manager service ${suffix}`;
