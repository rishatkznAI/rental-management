...
## Draft PR 233 P1 remediation evidence

The hostile-review remediation closes the six PR9a P1 findings without widening the
authorization boundary:

1. PR8 admission independently reconstructs the complete eight-table evidence graph,
   including membership, ordering, counts, candidate/due-date bindings,
   reconciliation dimensions, diagnostics, operation/audit bindings, finalization
   seal, freshness, and a post-insert reread. Any mismatch is classified as
   `PR8_EVIDENCE_MISMATCH` through Algorithm C with zero eligible-event DML.
2. Every registered non-authority PR9a denial type is persisted through Algorithm C
   using its exact type-specific projection and the shared authority snapshot
   projection. The registry contains the 16 contract literals and does not accept a
   prefix or arbitrary caller-supplied type.
3. The locked source state machine proves predecessor/root and successor/current
   revision structure before classification. Posting has precedence over eligibility,
   which has precedence over a changed pre-posting revision; replacement evidence is
   bound by the exact replacement-relation hash.
4. Algorithm C rereads the durable authority, PR8, PR6, source, event, operation,
   runtime-policy, and runtime-configuration evidence and independently reconstructs
   the denial package. It does not trust the Algorithm A projection or its hashes.
5. Runtime artifact, source-commit, configuration, and policy identity are supplied
   by a repository-owned branded contract whose exported default is disabled. Drift
   in each identity is covered independently for all three authority kinds.
6. The one nondeterministic identifier is `denialAttemptId`, generated by exactly one
   `node:crypto.randomUUID({ disableEntropyCache: true })` call on a new attempt.
   Event, correlation, conflict, and conflict-correlation IDs are deterministic and
   domain separated; exact replay and recovery consume no UUID.

The remediation tests include self-consistent but malformed caller projections,
hostile mutations across the PR8 graph, source lineage ambiguity and correction
precedence, runtime identity drift, deterministic identity separation, UUID
call-count assertions, exact replay, recovery, and zero-DML rejection paths.
...
- targeted PR9a suites: 95 tests passed, 0 failed;
- first and second mandated final-tree `npm test` passes: 2,438 tests passed, 0 failed in each;
- final-tree explicit `node --test tests/*.test.js`: 2,438 tests passed, 0 failed;
