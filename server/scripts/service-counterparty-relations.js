...
function printUsage() {
  console.log([
    'Usage:',
    '  node server/scripts/service-counterparty-relations.js [--dry-run] [--db /path/to/app.sqlite]',
    '  node server/scripts/service-counterparty-relations.js --apply [--db /path/to/app.sqlite]',
    '',
    'Default mode is a machine-readable dry-run and performs no writes.',
    '--apply creates a SQLite backup and atomically writes deterministic counterpartyId repairs only.',
    'Repairs use stable Service clientId, rentalId, objectId, or contractId chains.',
    'Names, INN, phone numbers, and other display metadata are never relation inputs.',
    'Unresolved or conflicting records are reported unchanged.',
  ].join('\n'));
}
...
async function main() {
  ...
  try {
    ...
    const preview = repairServiceCounterpartyRelations({
    ...
    const backupPath = path.join(
      path.dirname(dbPath),
      'backups',
      `pre-service-counterparty-${new Date().toISOString().replace(/[:.]/g, '-')}.sqlite`,
    );
    fs.mkdirSync(path.dirname(backupPath), { recursive: true });
    await db.backup(backupPath);
    const result = repairServiceCounterpartyRelations({
      readData: storage.readData,
...
if (require.main === module) {
  main().catch(error => {
    console.error(JSON.stringify({
      ok: false,
      code: error?.code || 'SERVICE_COUNTERPARTY_MIGRATION_FAILED',
