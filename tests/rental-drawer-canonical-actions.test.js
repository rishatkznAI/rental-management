...
test('document tab lists linked rental documents and opens create with canonical context', () => {
  ...
  assert.match(drawerSource, /const createRentalDocumentUrl = \(type: RentalDocumentCreateType\)/);
  assert.match(drawerSource, /params\.set\('rentalId', canonicalRentalId\)/);
  assert.match(drawerSource, /buildDocumentControl/);
  assert.match(drawerSource, /const chainContract = chainDocument/);
  assert.match(drawerSource, /const chainSpecification = chainDocument/);
  assert.match(documentsBlock, /Документы аренды/);
  assert.match(documentsBlock, /Контроль договора, спецификации и актов по этой аренде/);
  assert.match(drawerSource, /label: 'Договор аренды'/);
  assert.match(drawerSource, /label: 'Спецификация'/);
  assert.match(drawerSource, /label: 'Акт передачи'/);
