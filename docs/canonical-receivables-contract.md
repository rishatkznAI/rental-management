...
**PR8 implementation branch / original implementation / remediation commit:** `codex/pr8-actual-source-eligibility-dry-run`; `90f0db113c475c2342a9197d8dbb96c1f5472116`; `d1aec5445f3f6eccafe7c21903b99d9b1cf09a38`
...
**Implementation status:** PR1, PR2, and PR3 are released within their recorded foundation-only boundaries. PR4 remains a design-approved historical gate and was not released. PR5 is released only as the neutral platform identity and fail-closed authorization foundation. **PR6: RELEASED — Billing Source Authority foundation only. PR7: RELEASED — Forecast Receivables Planning foundation only. PR8: REMEDIATED FOR REVIEW — NOT RELEASED.** PR8 is an isolated, production-unreachable diagnostic foundation. Canonical production reads and writes, production settlement, production identity/bootstrap/source activation, production forecast calculation/read enablement, production actual-source evaluation, Finance, Dashboard, or Company Health/Risks switching, deployment, and cutover remain disabled, blocked, or unperformed.
...
## PR8 implementation-review record

**PR8: REMEDIATED FOR REVIEW — NOT RELEASED**

Remediation commit `d1aec5445f3f6eccafe7c21903b99d9b1cf09a38` on branch `codex/pr8-actual-source-eligibility-dry-run` preserves migration `actual_source_eligibility_dry_run_pr8` version 1 and exactly eight append-only diagnostic tables. It binds signature decisions to the exact conducted source policy identity/scope, validates a complete canonical adapter-ownership manifest, fully rereads and rehashes every persisted child and sealing row, freshly reauthorizes PR5 read authority before every query, and enforces the exact activation/same-run FK graph. The isolated engine still reads only the complete PR6 relational source universe, applies an explicit versioned 15-gate manifest, records every candidate check/blocker and six exact net/VAT/gross reconciliation dimensions, and classifies exact source slices only as `eligible_candidate` or `blocked`.
...
Future runtime activation additionally requires a separate governed execution capability, admission/rate and concurrency controls, storage telemetry, and approved retention/legal-hold controls. `receivables.read` is acceptable only inside this isolated foundation/test boundary and does not approve a runtime execution contract.

No production actual-source dry run was performed. Test-fixture zero-delta evidence does not satisfy the production PR8 reconciliation gate. The full implementation, migration, source, authorization, idempotency, concurrency, fresh-database, test, exclusion, and rollback evidence is recorded in `docs/actual-source-eligibility-pr8-audit.md`.
...
| PR8 source dry run | **PR8: REMEDIATED FOR REVIEW — NOT RELEASED** | Isolated diagnostic tables/engine/tests only; production policy, source data/run, approvals, activation, canonical writes, adapters, consumers, deployment, and release remain blocked |
...
- **PR8: REMEDIATED FOR REVIEW — NOT RELEASED.** Its schema, fail-closed engine, internal review reads, and artificial-fixture evidence include the independent-review remediation, but every production source/policy/approval/activation/reconciliation/runbook/release gate remains blocked. No production actual-source dry run was performed, and fixture evidence is not production evidence.
- **PR9 remains blocked** by all actual-source sufficiency, reconciliation, exact activation, legal/accounting, idempotency/replay, and zero-unexplained-delta gates, plus separate explicit canonical-write authorization. PR8 creates no posting event or authorization.
...
| PR8 | Source-authority dry run and discrepancy/quarantine reporting | **IMPLEMENTED FOR REVIEW — NOT RELEASED**; writes only its own append-only diagnostic tables, with no canonical/source/forecast/legacy writes or production entrypoint | Production policy/source population, actual dry-run evidence, approvals, zero unexplained delta, runbooks, and release remain required |
