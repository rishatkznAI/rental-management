...
**PR6 implementation PR / reviewed head / released squash commit:** [#212](https://github.com/rishatkznAI/rental-management/pull/212); `b2687e5c5c75caf21cf1d5659f687fcf2ba90f89`; `485808d24b8c5f6481e0520eec5c8985b71ffeab`

**PR6 migration / bounded scope:** `billing_source_authority_pr6`, version `1`; 16 append-only Billing Source Authority tables.

**PR7 implementation PR / reviewed head / released squash commit:** [#214](https://github.com/rishatkznAI/rental-management/pull/214); `c9bd77ddd5398af801e8781dbd2602b862703333`; `cb90e09f26c5b9916a4818fd96048070a6a1a662`

**PR7 migration / bounded scope:** `forecast_receivables_planning_pr7`, version `1`; 8 append-only Forecast Receivables Planning tables.

**PR8 implementation PR / reviewed head / released squash commit:** [#216](https://github.com/rishatkznAI/rental-management/pull/216); `fb66d27208dc810d5ccf60e11b3a5498daf7239d`; `afeac4de9b2711d9c6493855a8e3632443844f61`
...
**Approved decisions:** D-01 through D-27 in `docs/canonical-receivables-decisions.md`. D-28 through D-35 are design records requiring durable named owner/external approval and are not part of the approved baseline.
...
Implementation PR [#212](https://github.com/rishatkznAI/rental-management/pull/212), independently reviewed at head `b2687e5c5c75caf21cf1d5659f687fcf2ba90f89`, was squash-merged as `485808d24b8c5f6481e0520eec5c8985b71ffeab` on 2026-07-18. Its `lightweight-pr-check` completed successfully. Final verification used Node `v20.20.2` and npm `10.9.8`: the focused PR6 suites passed 135 tests including 3 independent-process concurrency remediation tests; the compatibility set passed 364 tests; two complete runs passed 2,073 tests each; and the production build passed. The released migration is `billing_source_authority_pr6` version 1 and creates exactly 16 append-only source-authority tables.

The release includes the P1 coverage-lifecycle remediation: active validated coverage has no mutable active flag, replacement identifies the exact predecessor set, cancellation/replacement append immutable lifecycle relations, and concurrent attempts have one atomic winner. It also includes the P1 evidence-integrity remediation: callers may only assert `expectedEvidenceSetHash`; the repository canonicalizes, persists, re-reads, and verifies the authoritative evidence hash transactionally.

The isolated **Billing Source Authority** foundation is limited to:
...
The implementation and audit are recorded in `docs/billing-source-authority-pr6-audit.md`. Release marks the bounded code and schema foundation only. It did not run production bootstrap, create production company/branch identity records or memberships/capability assignments, create activation-boundary/rental-line/UPD/coverage business rows, wire a legacy adapter, import history, backfill, dual-write, deploy, or cut over production. The production canonical resolver remains unconditional `null`; `CANONICAL_RECEIVABLES_READ_API_ENABLED` remains default-disabled; canonical financial tables remain empty. PR6 remains unconnected to production behavior and may not:
...
**PR7: RELEASED — Forecast Receivables Planning foundation only.**

Implementation PR [#214](https://github.com/rishatkznAI/rental-management/pull/214), independently reviewed at head `c9bd77ddd5398af801e8781dbd2602b862703333`, was squash-merged as `cb90e09f26c5b9916a4818fd96048070a6a1a662` on 2026-07-19. Its `lightweight-pr-check` completed successfully. The released migration remains `forecast_receivables_planning_pr7` version 1 and creates exactly 8 append-only planning tables for immutable runs, input snapshots/events, items, diagnostics, supersessions, operations, and audit. Persisted input lineage and business hashes are repository-owned; exact coverage/service containment, transactional PR5 authorization, PR6 source revalidation, concurrency, and rollback safeguards remain enforced.

Release marks only the isolated Forecast Receivables Planning foundation. It is not production activation, a production calculation adapter, read-API enablement, canonical debt, actual receivables, a closed-unbilled lane, combined three-lane reporting, `totalExpectedClientLoadMinor`, a Finance/Dashboard/Company Health/Risks switch, deployment, cutover, or PR8. `FORECAST_RECEIVABLES_READ_API_ENABLED` remains false by default, the production forecast resolver remains unconditional `null`, and production calculation remains unreachable. No canonical financial or PR6 source business rows were created.

Production VAT, rounding, minimum-term, event-authority/precedence, accountant/legal, activation, and consumer gates remain unresolved and fail closed. The approved indefinite retention/no-delete rule remains in force. The later PR8 diagnostic foundation does not retroactively expand or activate PR7.

## PR8 release record

**PR8: RELEASED — Actual-Source Eligibility Dry Run foundation only.**

Implementation PR [#216](https://github.com/rishatkznAI/rental-management/pull/216), independently reviewed at head `fb66d27208dc810d5ccf60e11b3a5498daf7239d`, was squash-merged with exact-head protection as `afeac4de9b2711d9c6493855a8e3632443844f61` on 2026-07-21 after `lightweight-pr-check` completed successfully. The released foundation preserves migration `actual_source_eligibility_dry_run_pr8` version 1 and exactly eight append-only diagnostic tables. It binds signature decisions to the exact conducted source policy identity/scope, validates the complete source-adapter ownership manifest, fully rereads and rehashes every persisted child and sealing row, freshly reauthorizes PR5 read authority before every query, and enforces scoped PR6 activation/same-run lineage. The isolated engine reads only the complete PR6 relational source universe, applies an explicit versioned fail-closed 15-gate manifest, persists candidates/checks/reconciliations/diagnostics plus append-only operation/audit sealing, and classifies exact source slices only as `eligible_candidate` or `blocked`.

`eligible_candidate` is testable diagnostic semantics only. Every persisted run/candidate is `diagnosticOnly = true`, `canonicalWriteAuthorized = false`, and `productionActivationAuthorized = false`; there is no canonical receivable ID, workflow status, `ActualReceivableEligibleV1`, posting outbox, source adapter, route, worker, scheduler, CLI, or frontend/consumer integration. Missing or incomplete accounting, evidence, signature, due-date, VAT, rounding, operational-event, correction, activation, cohort, or adapter decisions remain unresolved and block candidates. The production policy registry is intentionally unavailable.

Production startup reaches only the PR8 schema initializer after PR7. Evaluation and internal review repositories remain unreachable from production runtime. PR5 `receivables.read` permits only a freshly revalidated, human, exact-company/concrete-branch diagnostic evaluation context and grants no canonical/source/forecast write or production activation.
...
contracts. D-34–D-35 separately record the foundation-deployment boundary and its
six independent authorization levels. None is an approved decision.
...
| Forecast boundary/details | Forecast remains planning-only. Three lanes/components, total equation, 30-day horizon, status allow-list, `planned_future`, confidence levels/reasons, indefinite history, separate advances, return-line boundary, and minimum-term/discount order are approved | Production VAT/rounding/minimum-term policy and authoritative downtime/extension/return input lifecycle; activation and consumer contracts | Production forecast calculation/activation and PR11 consumers; not the released fail-closed PR7 foundation |
...
| PR9a disabled eligibility foundation | **IMPLEMENTATION AUTHORIZED — DISABLED FOUNDATION ONLY** | Gate B permits the additive seven-table schema, immutable authority/serialization contracts, Algorithm A eligible-event production and required denial/recovery foundation only; no Algorithm B, canonical business DML, deployment, activation, production read/write, backfill, or dual write |
| PR9b / complete PR9 canonical adapter | **NOT AUTHORIZED** | Canonical posting Algorithm B, production evidence/adapters, canonical writes, operational activation, deployment, and every later gate remain blocked |
...
No gate outcome authorizes product behavior beyond its named PR scope. PR1, PR2, PR3, PR5, and PR6 RELEASED status covers only their recorded foundations; all remain outside canonical production business reads and writes.
...
- **PR6: RELEASED — Billing Source Authority foundation only.** Its isolated foundation covers closed billing periods, immutable billing snapshots, an explicit conducted-UPD lifecycle, stable UPD line IDs, and deterministic line-to-period mappings. Production use remains gated by the applicable close/reopen, conducted-UPD, signature, mapping/granularity, VAT/rounding, correction/cancellation, return-source, activation, adapter, and accountant/legal requirements.
- **PR7: RELEASED — Forecast Receivables Planning foundation only.** The bounded foundation is documented in `docs/forecast-receivables-planning-pr7-audit.md`. Production policies/adapters remain absent, unresolved VAT/rounding/minimum-term and downtime/extension/return authority remain fail-closed, reads remain default-disabled behind an unconditional-null resolver, and canonical writes, combined reporting, Finance/Dashboard/Company Health/Risks switching, and production activation remain forbidden.
- **PR8: RELEASED — Actual-Source Eligibility Dry Run foundation only.** Its exact eight-table schema, fail-closed engine, internal diagnostic reads, persisted evidence/sealing, and artificial-fixture verification are released only inside the default-disabled foundation boundary. Every production source/policy/approval/activation/reconciliation/runbook gate remains blocked. No production actual-source dry run was performed, and fixture evidence is not production evidence.
- **PR9a implementation is authorized only as a disabled foundation.** Its boundary is migration `canonical_actual_posting_pr9` v1, immutable authority/authorization/activation and serialization contracts, Algorithm A `ActualReceivableEligibleV1`, exact replay, and required denial/recovery foundation. This creates no application entrypoint and authorizes no deployment, production migration, live adapter, Algorithm B, canonical business DML, production read/write, activation, settlement, shadow-read, or cutover. PR9b and complete PR9 remain unauthorized. PR8 itself creates no posting event or authorization.

PR6 created no canonical receivable, performs no canonical write, does not run PR8 eligibility or canonical posting, perform settlement/payment allocation, enable the canonical read API or forecast, switch Finance or Company Health/Risks, run production bootstrap, deploy, cut over production, or implement PR7. Future eligibility, canonical posting, settlement, and production activation remain inactive and require their own later gates.
...
| PR6 | Closed billing period, conducted UPD, stable lines, mapping, commercial-term, due-date, money/VAT, correction, and audit source authority | **PR6: RELEASED — Billing Source Authority foundation only.** No canonical projection or production activation | Authoritative lifecycle and evidence contract implemented and independently verified |
...
| PR8 | Source-authority dry run and discrepancy/quarantine reporting | **RELEASED — FOUNDATION ONLY**; writes only its own append-only diagnostic tables, with no canonical/source/forecast/legacy writes or production entrypoint | Production policy/source population, actual dry-run evidence, approvals, zero unexplained delta, runbooks, execution capability, operational controls, and activation remain required |
| PR9a | Disabled canonical actual posting foundation | Additive schema, authority contracts, Algorithm A eligibility event and required denial/recovery only; no canonical business DML | Gate B authorized implementation only; all production and deployment authority remains false |
| PR9b / PR9 | Forward-only canonical projection adapter | Not authorized; Algorithm B, production posting, backfill and dual write absent | Separate authorization, production evidence, owner-controlled activation boundary and rollback proof |
