...
| D-28 | Production evidence acceptance | Current immutable redacted pack with deployment/DB/identity/source/PR8/controls/approval lineage; missing means deny | No owner/external acceptance; recovery pack complete, but identity/source/PR8 evidence remains missing and ingress fails | design recorded; approval required | PR9 implementation/deployment/writes |
...
- **Current factual result:** The complete read-only recovery pack verifies active PR3 SHA `6a38582f5f90b85734884b6b12ad8e306b24619e`, exact empty PR1/PR2 financial tables and absent PR5–PR8 production schema/authority/run rows. Public ingress still times out, the pack has no independent acceptance, and production PR8 evidence is missing. See `docs/pre-pr9-production-evidence-pack.md`.
...
- **Current factual result:** First-start migration, failure-matrix and previous-code rollback simulations remain successful. PR #221 closed the repeated-startup defect: repeated startup preserves the exact `documents_gantt_shadow_indexes.applied_at`, registered drift fails closed without repair, and `repeatedStartupPassed = TRUE`. Public ingress is unhealthy; an approved durable production backup, accepted restore drill, approved storage threshold, pinned artifact, approved smoke plan and owner/release approval are missing. The readiness result remains `FOUNDATION_DEPLOYMENT_BLOCKED` and `foundationDeploymentAuthorized = FALSE`.
