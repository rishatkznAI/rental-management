...
**PR8: REMEDIATED FOR REVIEW — NOT RELEASED.** Migration `actual_source_eligibility_dry_run_pr8` v1 and its exact eight append-only diagnostic tables remain unchanged in identity and boundary. Remediation commit `d1aec5445f3f6eccafe7c21903b99d9b1cf09a38` adds exact signature-policy/source binding, complete adapter-ownership lineage, complete persisted child/seal recomputation, fresh PR5 authorization for every diagnostic read, and an exact scope/same-run FK graph. See `docs/actual-source-eligibility-pr8-audit.md`.
